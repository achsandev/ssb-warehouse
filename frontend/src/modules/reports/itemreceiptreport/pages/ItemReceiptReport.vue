...
const store = useItemReceiptReportStore()
...
const options = ref<TableParams>({
    ...
    sortBy: [{ key: 'receipt_date', order: 'desc' }], search: '',
})
...
const headers = computed<DataTableHeader[]>(() => [
    ...
    { title: t('qtyReceived'), key: 'qty_received', align: 'end', nowrap: true, sortable: false },
...
const canExport = computed(() => can('export', 'item_receipt_report') || can('manage', 'all'))
...
<template>
    <v-card>
        ...
        <v-card-text>
            ...
            >
                <template #item.receipt_date="{ item }">{{ formatDate(item.receipt_date as string) }}</template>
                <template #item.qty="{ item }">{{ money(item.qty) }}</template>
                <template #item.qty_received="{ item }">{{ money(item.qty_received) }}</template>
