...
import { stockUsageColumns } from '@/modules/reports/_shared/reportColumns'
import { useStockUsageReportStore } from '@/stores/stock_usage_report'
...
const store = useStockUsageReportStore()
...
const options = ref<TableParams>({
    page: 1,
    itemsPerPage: 10,
    sortBy: [{ key: 'usage_date', order: 'desc' }],
    search: '',
})
...
const headers = computed<DataTableHeader[]>(() => [
    ...
    { title: t('usageQty'), key: 'usage_qty', align: 'end', nowrap: true, sortable: false },
...
const canExport = computed(() => can('export', 'stock_usage_report') || can('manage', 'all'))
...
<template>
    <v-card>
        ...
        <v-card-text>
            ...
            >
                <template #item.usage_date="{ item }">{{ formatDate(item.usage_date as string) }}</template>
                <template #item.project_name="{ item }">{{ item.project_name ?? '-' }}</template>
                <template #item.qty="{ item }">{{ Number(item.qty).toLocaleString('id-ID') }}</template>
                <template #item.usage_qty="{ item }">{{ Number(item.usage_qty).toLocaleString('id-ID') }}</template>
