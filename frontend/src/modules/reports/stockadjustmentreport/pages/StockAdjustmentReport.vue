...
const options = ref<TableParams>({
    ...
    sortBy: [{ key: 'adjustment_date', order: 'desc' }],
    search: '',
})
...
const headers = computed<DataTableHeader[]>(() => [
    ...
    { title: t('rack'), key: 'rack_name', align: 'start', nowrap: true },
    { title: t('adjustmentQty'), key: 'adjustment_qty', align: 'end', nowrap: true, sortable: false },
...
const canExport = computed(() => can('export', 'stock_adjustment_report') || can('manage', 'all'))
...
<template>
    <v-card>
        ...
        <v-card-text>
            ...
            >
                <template #item.adjustment_date="{ item }">{{ formatDate(item.adjustment_date as string) }}</template>
                <template #item.adjustment_qty="{ item }">{{ Number(item.adjustment_qty).toLocaleString('id-ID') }}</template>
