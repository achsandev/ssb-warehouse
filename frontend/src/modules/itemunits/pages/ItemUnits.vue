...
import ItemUnitsForm from '@/modules/itemunits/components/ItemUnitsForm.vue'
...
const formFields = reactive<ItemUnitsForm>({
    id: 0,
    uid: '',
    name: '',
    symbol: '',
...
const headers = computed<DataTableHeader[]>(() => [
    ...
    { title: t('unitName'), key: 'name', align: 'start' },
    { title: t('symbol'), key: 'symbol', align: 'start' },
...
const handleActionMenu = (action: string, data: ItemUnitsList) => {
    if (action === 'detail') {
...
const handleOpenDialog = (item: ItemUnitsList | null) => {
    if (item) {
        formAction.value = 'update'
        formFields.id = item.id
        formFields.uid = item.uid
        formFields.name = item.name
        formFields.symbol = item.symbol
    } else {
