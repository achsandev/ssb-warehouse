...
import PaymentMethodsForm from '@/modules/paymentmethods/components/PaymentMethodsForm.vue'
...
const formFields = reactive<PaymentMethodsForm>({
    id: 0,
    uid: '',
    name: '',
    description: ''
...
const handleActionMenu = (action: string, data: PaymentMethodsList) => {
    if (action === 'detail') {
...
const handleOpenDialog = (item: PaymentMethodsList | null) => {
    if (item) {
...
const handleOpenDetailDialog = (item: PaymentMethodsList) => {
    detailData.value = []
    resetForm()
    try {
