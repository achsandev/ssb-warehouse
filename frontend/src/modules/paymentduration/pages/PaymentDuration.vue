...
import PaymentDurationForm from '@/modules/paymentduration/components/PaymentDurationForm.vue'
...
const store = usePaymentDurationStore()
...
const formField = reactive<PaymentDurationList>({
    uid: '',
    name: '',
    created_at: '',
    updated_at: '',
...
// Functions
// =========================
const handleActionMenu = (action: string, data: PaymentDurationList) => {
    if (action === 'detail') {
...
const handleOpenDialog = (item: PaymentDurationList | null) => {
    if (item) {
...
const handleOpenDetailDialog = (item: PaymentDurationList) => {
    detailData.value = []
    resetForm()
    try {
        Object.assign(formField, item)
...
const handleSubmit = (value: PaymentDurationForm) => {
    if (formAction.value === 'create') {
        store.create(value)
    } else {
        if (formField.uid) {
            store.update(formField.uid, value)
        }
    }
    handleClose()
}
