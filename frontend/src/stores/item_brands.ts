import { create, destroy, get, update } from '@/api/item_brands'
import { defineStore } from 'pinia'
import { useMessageStore } from './message'

const message = useMessageStore()

export const useItemBrandsStore = defineStore('itemBrands', {
    state: () => ({
        data: [] as ItemBrandsList[],
        pagination: {
            page: 1,
            per_page: 5,
            last_page: 1
    ...
    actions: {
        ...
        async update(uid: string, payload: ItemBrandsForm) {
            this.loading = true
            try {
