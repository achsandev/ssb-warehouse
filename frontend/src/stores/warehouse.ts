import { create, destroy, get, getBasicInfoByUid, update } from '@/api/warehouse'
...
export const useWarehouseStore = defineStore('warehouse', {
    state: () => ({
        currentWarehouse: null as { uid: string, name: string } | null,
        data: [] as WarehouseList[],
        pagination: {
            page: 1,
            per_page: 5,
            last_page: 1
    ...
    actions: {
        ...
        async create(payload: WarehouseForm) {
            this.loading = true
            try {
        ...
        async update(uid: string, payload: WarehouseForm) {
            this.loading = true
            try {
        ...
        setCurrentWarehouse(payload: { uid: string, name: string }) {
            this.currentWarehouse = payload
        },
