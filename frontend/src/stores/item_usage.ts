...
import { create, destroy, get, update } from '@/api/item_usage.api'

const message = useMessageStore()

export const useItemUsageStore = defineStore('itemUsage', {
    state: () => ({
        data: [] as ItemUsageList[],
        pagination: {
            page: 1,
            per_page: 5,
            last_page: 1,
    ...
    actions: {
        ...
        async create(payload: ItemUsageForm) {
            this.loading = true
            this.error = false
            try {
        ...
        async update(uid: string, payload: Partial<ItemUsageForm> & { status?: string }) {
            this.loading = true
            this.error = false
            try {
