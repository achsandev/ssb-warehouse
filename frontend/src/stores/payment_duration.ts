import { create, destroy, get, update } from '@/api/payment_duration'
import { defineStore } from 'pinia'
import { useMessageStore } from './message'

const message = useMessageStore()

export const usePaymentDurationStore = defineStore('paymentDuration', {
    state: () => ({
        currentItems: null as { uid: string, name: string } | null,
        data: [] as PaymentDurationList[],
        pagination: {
            page: 1,
            per_page: 5,
            last_page: 1
    ...
    actions: {
        ...
        async create(payload: PaymentDurationForm) {
            this.loading = true
            try {
        ...
        async update(uid: string, payload: PaymentDurationForm) {
            this.loading = true
            try {
