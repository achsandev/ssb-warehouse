import { create, destroy, get, getByUid, update } from '@/api/rack'
...
export const useRackStore = defineStore('rack', {
    ...
    actions: {
        ...
        async getByUid(uid: string | string[], payload?: TableParams) {
            this.loading = true
            try {
                const params: any = {
                    page: payload?.page || 1,
                ...
                const data = await getByUid(uid, params)
        ...
        async update(uid: string, payload: RackForm) {
            this.loading = true
            try {
