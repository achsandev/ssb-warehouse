...
import { create, destroy, get, update } from '@/api/return_item.api'

const message = useMessageStore()

export const useReturnItemStore = defineStore('returnItem', {
    state: () => ({
        data: [] as ReturnItemList[],
        pagination: {
            page: 1,
            per_page: 10,
            last_page: 1,
    ...
    actions: {
        ...
        async create(payload: ReturnItemForm) {
            this.loading = true
            this.error = false
            try {
        ...
        async update(uid: string, payload: Partial<ReturnItemForm> & { status?: string }) {
            this.loading = true
            this.error = false
            try {
