...
import { create, destroy, get, update } from '@/api/cash_payment.api'

const message = useMessageStore()

export const useCashPaymentStore = defineStore('cashPayment', {
    state: () => ({
        data: [] as CashPaymentList[],
        pagination: {
            page: 1,
            per_page: 5,
            last_page: 1,
    ...
    actions: {
        ...
        async create(payload: CashPaymentForm) {
            this.loading = true
            this.error = false
            try {
        ...
        async update(uid: string, payload: Partial<CashPaymentForm> & { status?: string }) {
            this.loading = true
            this.error = false
            try {
